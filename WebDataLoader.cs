...
namespace WebDataLoader
{
    public class WebImporter
    {
        ...
        public enum TaskStatus{ done, progress, fail};
        public TaskStatus GetImportStatus(string WebServer, string WebServerU, string WebServerP, int id)
        {
            object Service = GetMigproSerice(WebServer, WebServerU, WebServerP);
            MethodInfo mi = Service.GetType().GetMethod("GetImportStatus");
            object[] argss = new object[1] { id };
            int res = Convert.ToInt32(mi.Invoke(Service, argss));
            if (res > 2)
            {
